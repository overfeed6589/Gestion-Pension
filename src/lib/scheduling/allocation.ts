 ...
 * Ne modifie la base que si l'unité est effectivement libre sur la période.
 */
export async function assignUnitToSegment(input: AssignUnitInput): Promise<AssignUnitResult> {
  const startStr = toDateString(input.startDate);
  const endStr = toDateString(input.endDate);
  ...
  return db.transaction(async (tx) => {
    ...
    // 2. Verrouille le segment cible et vérifie la cohérence booking.
    const [segment] = await tx
      .select({ id: bookingSegments.id, bookingId: bookingSegments.bookingId })
      .from(bookingSegments)
